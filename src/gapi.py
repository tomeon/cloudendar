...
# NATIVE_CLIENT_SECRET_FILE is name of a file containing the OAuth 2.0 information for
...
NATIVE_CLIENT_SECRET_FILE = os.path.join(os.path.dirname(__file__),
...
def get_flow_from_clientsecrets(client_secret=NATIVE_CLIENT_SECRET_FILE, scope=APP_SCOPE):
...
class CalendarAPI(GAPI):
    ...
    def query_calendars_free(self, users, start_time=None, end_time=None):
        """ Returns a calendar of the form returned by a freebusy query,
            ...
            @returns: Returns a dictionary of calendars containing free and busy times
                for each user, keyed to the user's EMAIL ADDRESS, not ONID.
        """
        start_time, end_time = self._format_start_end(start_time, end_time)
    ...
    def get_calendars(self, calendars=None, tz=None):
        """ Returns the CalendarAPI object's 'calendar' attribute, after
            optionally converting its timezone

            @param tz: a 'tzinfo' object used to convert the calendar's timezone

            @return: Returns the object's 'calendar' attribute, possibly with
                timezone converted.
        """
    ...
    def get_ranges_overlaps(self, users=None, start_time=None, end_time=None,
        ...
        """ Returns a list of dictionaries containing permutations of free
            ...
            @param calendars: a list of calendars of the same structure as
                those extracted from the dictionary returned by
                apiclient.discovery.build().freebusy.query().execute()
            @param tz: a tzinfo object used for converting the calendars' timezone
            @param status: a string, either 'free' or 'busy'
            @param whole: a Boolean value.  If true, it returns results for
                only those calendars whose users are free for the entire interval,
                i.e. from start_time to end_time.

            @return: Returns a list of dictionaries of the form: [{'start':
                <datetime>, 'end': <datetime>, 'onids':
                ['user1@onid.oregonstate.edu', 'user2@onid.oregonstate.edu', ...]}].
        ...
        if convert_func is None:
            ranges_overlaps = [
                ...
                'start': segment.interval.lower,
                'end': segment.interval.upper}
                for segment in ranges_map
            ]
        else:
            ranges_overlaps = [
                {'onids': [u for u in segment.value],
                'start': convert_func(segment.interval.lower),
                'end': convert_func(segment.interval.upper)}
                for segment in ranges_map
            ]

        # Return only those intervals when all users are free
        if whole:
            onid_set = set(self.onids)
            return [r for r in ranges_overlaps if set(r.get('onids')) ==
                    onid_set]

        # If the set of free time ranges is larger than one, we know that
        # nobody is free for the whole interval.
        if duration:
            if len(ranges_overlaps) > 1:
                return []
    ...
    def to_tz(self, tz, dt):
        """ Converts a datetime object to a different timezone
            ...
            @param tz: a tzinfo object used for converting the timezone
            @param dt: a datetime object whose timezone will be converted

            @return: Returns 'dt' with its timezone converted.
        """
        if tz is None:
            tz = self.tz
    ...
    def _convert_tz(self, tz=tzutc):
        """ Curried function used for iterating over a list of datetime objects,
            converting them to the tzutc() timezone by default, or optionally some
            other timezone.

            @param tz: a tzinfo object

            @return: Returns a function of one argument used for iterating over
    ...
    def _datetimes_to_utc(self, *args):
        """ Converts its arguments to the tzutc() timezone

            @params *args: a group of datetime objects

            @returns: Returns the *args as a list, with timezone converted
        """
        ret = []
        for dt in args:
    ...
    def _format_start_end(self, start_time, end_time):
        """ Provides a sane default for start and end times

            @param start_time: a datetime object representing the start of the
                interval of interest
            @param end_time: a datetime object representing the end of the
                interval of interest

            @return: Returns start_time and end_time in a list
        """
...
# TODO: get admin rights and set this up for more efficient querying of users
# in the OSU domain.
