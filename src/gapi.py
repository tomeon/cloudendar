...
#def execute_query(self, service=None, start_time, end_time, users):
def run_query(func):
...
class CalendarAPI(GAPI):
    ...
    def build_freebusy_query(self, timeMin, timeMax, onids, timeZone=None,
    ...
    # Decorates function with special try-except block
    @run_query
    def run_freebusy_query(self, start_time, end_time, users):
        start_time_str = generate(start_time)
        end_time_str = generate(end_time)

        freebusy_api = self.service.freebusy()
        query = self.build_freebusy_query(
            start_time_str, end_time_str, users)
        request = freebusy_api.query(body=query)
        return request.execute()


    def get_calendars_free(self, start_time, end_time, users):
        start_time = start_time.replace(tzinfo=pytz.utc)
        end_time = end_time.replace(tzinfo=pytz.utc)

        freebusy = self.run_freebusy_query(start_time, end_time, users)

        calendars = self.get_calendars(freebusy)

        # Convert busy intervals from strings to datetime objects
        new_calendars = self.convert_calendars(calendars, ['busy'],
                                               self.get_ranges_datetime_obj)

        # Add list of free times to calendars
        return self.calendars_free(start_time, end_time, new_calendars)


    def convert_ranges_dict(self, convert_func, ranges_list):
        """
        Pre:
    ...
    def get_calendars(self, freebusy):
        return freebusy.get('calendars')


    def disc_interval_set(self, pairs):
        """
        Pre:
    ...
    def interval_set_to_list(self, interval_set):
        """
        Pre:
    ...
    def convert_calendars(self, calendars, statuses, convert_func):
        """ Pre:
    ...
    def ranges_overlaps(self, calendars, status):
        """
        Pre:
            - calendars :
        """
    ...
    def calendars_free(self, timeMin, timeMax, calendars):
        calendars_local = calendars.copy()
        ...
        for account, ranges_dict in calendars_local.iteritems():
            ...
            if busy_ranges == []:
                ...
                busy_ranges_intervals = self.disc_interval_set(busy_ranges_tuples)
            ...
            free_ranges = self.interval_set_to_list(free_ranges_intervals)
